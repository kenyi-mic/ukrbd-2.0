...
const ProductDetails = () => {
  ...
  return (
    <View key={id} style={styles.container}>
      <View className="flex flex-row justify-between px-3 pb-4  bg-gray-400 w-full">
        {/*back bottom*/}
        <TouchableOpacity
        ...
        >
          ...
          <Text className="absolute  text-sm text-gray-600 font-bold bg-green-500 w-5 h-5  rounded-full text-center ">
            {items.length}
          </Text>
        </TouchableOpacity>
      </View>
      ...
      <View className="ml-3">
        <Text className="my-2 text-xl font-semibold text-gray-600">{name}</Text>
        <Text numberOfLines={5} className="text-sm font-semibold text-gray-400">
          {description}
        </Text>
        {/* Rating*/}
