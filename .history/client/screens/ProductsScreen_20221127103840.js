...
const ProductsScreen = ({ navigation, route }) => {
  ...
  return (
    <SafeAreaView style={styles.container}>
      <ProductsHeader />
      <ScrollView>
        <View className="relative" key={routeItem?.id}>
          <Image
            className="w-full h-56 bg-gray-100 p-4"
            source={{ uri: urlFor(routeItem?.imgUrl).url() }}
          />
        </View>
        <View className="bg-white">
          <View className="px-4 py4">
            <Text className="text-3xl font-bold">{routeItem?.title}</Text>
