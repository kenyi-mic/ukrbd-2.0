...
router.get("/", async (req, res) => {
  ...
  try {
    ...
    if (qNew) {
      ...
      let cat = Category.aggregate(categories);
      cat.toArray().then((objects) => {
        console.loga("Output ", objects);
        objects.forEach((object) => {
          if (object.products > 0) {
            console.log(object.title, object.image, object.desc);
          }
        });
      });
    } else {
